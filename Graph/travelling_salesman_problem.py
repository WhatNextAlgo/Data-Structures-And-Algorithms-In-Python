class TravellingSalesmanProblem:
    def __init__(self,graph):
        ...
        self.hamiltonain_cycles = []
        # track what noded (vertices) are included in the cycle
    ...
    def is_valid(self,vertex,actual_position):
        ...
        # whether the given vertex has been already visited
    ...
    def tsp(self,actual_position,counter,cost):
        ...
        # we have considered all the nodes i in the G(V,E)
        # and the last node can be connected to first node (form a cycle)
        if counter == self.n and self.graph[actual_position][0]:
            self.path.append(0)
            print(self.path)
            self.hamiltonain_cycles.append(cost + self.graph[actual_position][0])
            self.path.pop()
            return
        
        # consider all the nodes in the G(V,E) graph (we filter out the non-adjacent nodes)
        for i in range(self.n):
            # check whether we include the node with index i to the path (cycle)
...
if __name__ == "__main__":
    ...
    tsp.tsp(0,1,0)
    print(min(tsp.hamiltonain_cycles))
